/**
 * Parse / validate LifeEventStep.target JSON and resolve public deep links.
...
import type { LifeEventStepTarget } from "./types";

export function parseStepTarget(raw: unknown): LifeEventStepTarget {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    return {};
  }
  const o = raw as Record<string, unknown>;
  const target: LifeEventStepTarget = {};
  ...
  if (o.listingFilters && typeof o.listingFilters === "object" && !Array.isArray(o.listingFilters)) {
    ...
    const filters: NonNullable<LifeEventStepTarget["listingFilters"]> = {};
...
export function resolveStepTargetHref(
  ...
  if (target.listingType === "property") {
    return appendQuery("/real-estate", {
      listingType: target.listingFilters?.listingType,
...
export function serializeStepTarget(target: LifeEventStepTarget): LifeEventStepTarget {
