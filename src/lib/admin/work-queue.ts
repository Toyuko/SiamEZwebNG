...
export type WorkQueueItem = {
  id: string;
  kind: "payment" | "workflow_approval" | "freelancer" | "company" | "listing_enquiry" | "vehicle_lead";
  title: string;
  href: string;
  ageHours: number;
};
...
export async function getWorkQueue(): Promise<WorkQueueItem[]> {
  const [payments, approvals, freelancers, companies, enquiries, vehicleLeads] = await Promise.all([
    ...
    prisma.vehicleLead
      .findMany({
        ...
        take: 30,
        select: { id: true, createdAt: true, leadNumber: true, displayTitle: true, type: true },
      })
      .catch(() => []),
  ]);
  return [
    ...
    ...vehicleLeads.map((row) => ({ id: row.id, kind: "vehicle_lead" as const, title: `${row.type === "sell" ? "SELL" : "FIND"} ${row.leadNumber} — ${row.displayTitle}`, href: `/admin/vehicle-leads/${row.id}`, ageHours: ageHours(row.createdAt) })),
